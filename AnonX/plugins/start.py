...
async def start_comm(client, message: Message, _):
    ...
    if len(message.text.split()) > 1:
        ...
        if name[0:4] == "help":
            ...
            await message.reply_sticker("CAACAgUAAxkDAAIjSGQygr6dsOvdcakCAzYW3Vl64J9BAAJqBwAC1LmZVTC6JInX0GiiLwQ")
        ...
        if name[0:3] == "sta":
            ...
            def get_stats():
                ...
                for vidid, count in list_arranged.items():
                    ...
                    if vidid == "telegram":
                        msg += f"🔗[ᴛᴇʟᴇɢʀᴀᴍ ᴍᴇᴅɪᴀ](https://t.me/ckanskuluy) ** ᴩʟᴀʏᴇᴅ {count} ᴛɪᴍᴇs**\n\n"
                    else:
        ...
        if name == "verify":
            await message.reply_text(f"Merhaba {message.from_user.first_name},\nKendinizi doğruladığınız için teşekkürler {config.MUSIC_BOT_NAME}, Şimdi geri dönüp beni kullanmaya başlayabilirsin..")
            if await is_on_off(config.LOG):
                sender_id = message.from_user.id
                sender_name = message.from_user.first_name
                return await app.send_message(
                    config.LOG_GROUP_ID,
                    f"{message.from_user.mention} Sadece botu başlattı <code>Kendini doğruladı</code>\n\n**ID:** {sender_id}\n**Kullanıcı İsmi:** {sender_name}",
                )
            return
        if name[0:3] == "inf":
            m = await message.reply_text("🔎")
...
🎥 **Kanal:** {channel}
📎 **Kanal Link:** [ᴠɪsɪᴛ ᴄʜᴀɴɴᴇʟ]({channellink})
...
💖 sᴇᴀʀᴄʜ ᴩᴏᴡᴇʀᴇᴅ ʙʏ {config.MUSIC_BOT_NAME}"""
            key = InlineKeyboardMarkup(
                [
                    [
                        ...
                        InlineKeyboardButton(
                            text="• sᴜᴩᴩᴏʀᴛ •", url="https://t.me/ckanskuluy"
                        ),
                    ],
                ]
            )
            ...
            if await is_on_off(config.LOG):
                ...
                return await app.send_message(
                    config.LOG_GROUP_ID,
                    f"{message.from_user.mention} Kontrol etmek için Bot'u çalıştırdı <code>Parça bilgisi.</code>\n\n**ID:** {sender_id}\n**Kullanıcı İsmi:** {sender_name}",
                )
    else:
        try:
        ...
        if await is_on_off(config.LOG):
            ...
            return await app.send_message(
                config.LOG_GROUP_ID,
                f"{message.from_user.mention} Botunuzu yeni çalıştırdı.\n\n**ID:** {sender_id}\n**Kullanıcı İsmi:** {sender_name}",
            )
